...
export default class ToolActionHandler {
    ...
    constructor(handlers: any, world: Convolvr, socket: any) {
        ...
        socket.on("tool action", (packet: any) => {
                ...
                case "Component Tool":
                    voxel.entities.map((voxelEnt: Entity) => {
                        ...
                        if (voxelEnt.id == data.entityId)
                            ...
                            voxelEnt.update(null, null, voxelEnt.components.concat(data.entity.components), null, null, {
                                ignoreRotation: true
                            });
                    });
                    break;
                ...
                case "Update Tool":
                    voxel.entities.map((voxelEnt: Entity) => {
                        ...
                        if (voxelEnt.id == data.entityId) {
                            ...
                            voxelEnt.update(null, null, null, data.components[0], data.componentPath, { ignoreRotation: true });
                        }
                    });
                    break;
                case "Delete Tool":
                    voxel.entities.map((voxelEnt: Entity, i: number) => {
                        // find & re-init entity ^^^^^^
                        if (voxelEnt.id == data.entityId) {
            ...
            if (world.settings.IOTMode) animate(world, Date.now(), 0);
        });
    }
}
