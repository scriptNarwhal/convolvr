...
export let animate = (world: Convolvr, last: number, cursorIndex: number) => {
    ...
    if (mode != "stereo") {
        ...
        requestAnimationFrame(() => {
            animate(world, time, cursorIndex);
        });
...
export let vrAnimate = (world: Convolvr, display: VRDisplay, time: number, oldPos: number[], cursorIndex: number) => {
    let now = Date.now(),
        delta = Math.min(now - time, 500) / 0.08,
        t = world.three,
        camera = t.camera,
        cPos = camera.position,
        frame = world.vrFrame,
    ...
    world.systems.time = now;

    if (world.HMDMode != "flymode") {
        // room scale + gamepad movement
    ...
    display.requestAnimationFrame(() => {
        vrAnimate(world, display, now, vrSpacePos, cursorIndex);
    }); // Keep looping.
};
