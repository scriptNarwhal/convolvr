...
func generateBuilding(id int, world string, x int, z int, altitude float32) *Entity {
	...
	titles = []string{"Vacant Building", "Office Tower", "Rename This", "Factory", "Armory", "Residence", "Generated", "Building", world + " HQ", "Venue", "Cafe"}
...
func generateBuildingWall(title string, w int, i int, floors int, width float64, structureSize float64) *Component {
	...
	if w < 2 {
		...
		if i == floors-1 {
			...
			wallPos = []float64{0.0, wallHeight + 0.59*structureSize, structureSize/2.0 + (float64(w)-1)*structureSize}
			geometry["merge"] = false
			geometry["size"] = []float64{structureSize * width, structureSize / 2.6, 5000}
