...
func generateNet(id int, world string, x int, z int, altitude float32) *Entity {
	var (
		structure           *Entity
	...
	structure = NewEntity(id+1, "Indra's Net", world, []int{x, 0, z}, structureComponents, structurePos, []float64{0.0, 0.0, 0.0, 0.0}, structureRadius, nil)
	return structure
}

func canPlaceNetAt(x int, y int, z int) bool {
